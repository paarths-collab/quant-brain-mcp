...
export default function TerminalLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-black text-white overflow-hidden font-mono">
      {/* Sidebar */}
      <aside
        className={`relative flex flex-col shrink-0 border-r border-white/[0.06] bg-white/[0.03] backdrop-blur-2xl transition-all duration-300 ${collapsed ? 'w-[60px]' : 'w-[240px]'
          }`}
      >
        {/* Logo */}
        <div className="flex items-center gap-3 px-4 py-5 border-b border-white/[0.05] h-[70px] shrink-0">
        ...
        {/* System Status */}
        {!collapsed && (
          <div className="p-5 border-t border-white/[0.05] space-y-2.5 bg-transparent">
            {[
      ...
      <main className="flex-1 overflow-auto relative bg-black">
        {/* Top bar */}
        <header className="sticky top-0 z-20 flex items-center justify-between h-[70px] px-8 border-b border-white/[0.06] bg-black/60 backdrop-blur-xl shrink-0">
        ...
        <div className="px-8 pt-0 pb-8 min-h-[calc(100vh-70px)]">
          {children}
        </div>
      </main>
    </div>
