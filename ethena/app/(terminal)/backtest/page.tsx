...
const STRATEGIES = [
  { id: 'momentum', name: 'Momentum', icon: TrendingUp, color: '#3b82f6', description: 'Price momentum signals', params: { lookback: 20 } },
  { id: 'mean_reversion', name: 'Mean Revert', icon: Activity, color: '#10b981', description: 'Deviation from MA', params: { window: 20, num_std: 2 } },
  { id: 'ema_crossover', name: 'EMA Cross', icon: Zap, color: '#f59e0b', description: 'Golden/Death cross EMAs', params: { fast: 12, slow: 26 } },
  { id: 'sma_crossover', name: 'SMA Cross', icon: Layers, color: '#06b6d4', description: '50/200 SMA crossover', params: { short_window: 50, long_window: 200 } },
  { id: 'macd', name: 'MACD', icon: Target, color: '#8b5cf6', description: 'MACD signal crossover', params: { fast: 12, slow: 26, signal: 9 } },
  { id: 'rsi_reversal', name: 'RSI Reversal', icon: BarChart3, color: '#ef4444', description: 'Oversold / overbought', params: { window: 14, lower: 30, upper: 70 } },
  { id: 'rsi_momentum', name: 'RSI Momentum', icon: ArrowUpDown, color: '#ec4899', description: 'RSI cross + trend filter', params: { rsi_window: 14, lower: 40, upper: 60 } },
  { id: 'breakout', name: 'Breakout', icon: Crosshair, color: '#14b8a6', description: 'Donchian channel break', params: { lookback: 20 } },
  { id: 'fibonacci_pullback', name: 'Fib Pullback', icon: GitBranch, color: '#a855f7', description: 'Fibonacci retracement', params: { lookback: 50 } },
  { id: 'support_resistance', name: 'Sup/Res', icon: Shield, color: '#f97316', description: 'S/R bounce trading', params: { lookback: 30, tolerance_pct: 0.01 } },
]
...
function MetricChip({ label, value, color = 'text-white' }: { label: string; value: any; color?: string }) {
  return (
    <div className="p-3.5 bg-white/[0.02] border border-white/[0.06] rounded-xl">
      <div className="text-[10px] font-mono text-white/25 uppercase tracking-widest mb-1">{label}</div>
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/backtest`, {
        ...
        body: JSON.stringify({ symbol, strategies: selectedStrategies, range: '1y', params, start: startDate || undefined, end: endDate || undefined }),
      })
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-mono font-bold text-white">Strategy Lab</h1>
          <p className="text-[12px] font-mono text-white/30 mt-0.5">Multi-strategy backtesting · Monte Carlo simulation · Parameter optimization</p>
        </div>
        <div className="flex bg-white/[0.03] rounded-lg p-1 border border-white/[0.07]">
          {['US', 'IN'].map(m => (
            <button key={m} onClick={() => { setMarket(m); setSymbol(m === 'IN' ? 'RELIANCE.NS' : 'AAPL') }}
              className={`px-4 py-1.5 rounded-md text-[12px] font-mono font-semibold transition-all ${market === m ? 'bg-orange-500 text-white' : 'text-white/40 hover:text-white'}`}>
              {m}
            </button>
          ))}
        </div>
      ...
      {/* Strategy Grid */}
      <div>
        <div className="text-[11px] font-mono text-white/30 uppercase tracking-widest mb-3">Select Strategies (Max 4)</div>
          ...
          {STRATEGIES.map(strat => {
            ...
            return (
              <button key={strat.id} onClick={() => toggleStrategy(strat.id)}
                className={`p-3.5 rounded-xl border transition-all text-left ${active ? 'border-orange-500/50 bg-orange-500/10' : 'border-white/[0.07] bg-white/[0.02] hover:bg-white/[0.05] hover:border-white/[0.14]'}`}>
                <Icon className="w-5 h-5 mb-2" style={{ color: strat.color }} />
                <div className="text-[12px] font-mono font-semibold text-white">{strat.name}</div>
                <div className="text-[10px] text-white/30 mt-0.5 leading-tight">{strat.description}</div>
              </button>
            )
          })}
        </div>
      ...
      <div className="flex flex-wrap gap-3 items-end">
        {[
          ...
          <div key={field.label} className="flex-1 min-w-[140px]">
            <label className="block text-[10px] font-mono text-white/30 uppercase tracking-widest mb-2">{field.label}</label>
            <input
              type={field.type} value={field.value} onChange={e => field.onChange(e.target.value)}
              style={{ colorScheme: 'dark' }}
              className="w-full px-3 py-2.5 bg-white/[0.03] border border-white/[0.08] rounded-xl text-white font-mono text-[13px] focus:outline-none focus:border-orange-500/30 transition-colors"
            />
          </div>
        ))}
        <div className="flex-1 min-w-[140px]">
          <label className="block text-[10px] font-mono text-white/30 uppercase tracking-widest mb-2">Initial Capital</label>
          <input
        ...
        <button onClick={handleRun} disabled={isRunning}
          className="flex items-center gap-2 px-6 py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-[13px] font-mono font-semibold disabled:opacity-50 transition-all shadow-lg shadow-orange-500/20">
          <Play size={15} />
        ...
        <div className="space-y-5">
          {results.isMulti ? (
            <>
              ...
              {/* Detail tabs */}
              <div className="p-5 rounded-xl border border-white/[0.07] bg-white/[0.02] space-y-5">
            ...
            <>
              ...
              {(results.trades || []).length > 0 && (
                <div className="p-5 rounded-xl border border-white/[0.07] bg-white/[0.02] overflow-x-auto">
