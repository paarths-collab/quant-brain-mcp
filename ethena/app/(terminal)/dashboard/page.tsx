...
const CARD = "group relative p-6 rounded-2xl border border-white/[0.07] bg-white/[0.03] backdrop-blur-xl hover:border-white/[0.13] hover:bg-white/[0.05] transition-all duration-500 overflow-hidden"
const CARD_GLOW = "group relative p-6 rounded-2xl border border-white/[0.07] bg-white/[0.03] backdrop-blur-xl shadow-[0_0_24px_rgba(79,70,229,0.06)] hover:shadow-[0_0_32px_rgba(79,70,229,0.12)] hover:border-indigo-500/25 hover:bg-white/[0.05] transition-all duration-500 overflow-hidden"
...
function IndexCard({ idx, symbol }: { idx: typeof INDICES_US[0]; symbol: string }) {
  ...
  return (
    <div className={CARD_GLOW}>
      ...
      <NeonSpark data={spark} color={BLUE_ACCENT} id={idx.symbol} />
    </div>
  )
}
...
function FredCard({ m }: { m: typeof FRED[0] }) {
  ...
  return (
    <div className={CARD_GLOW}>
      ...
      <NeonLine data={data} color={BLUE_ACCENT} id={m.id} unit={m.unit} />
    </div>
  )
}
  ...
  return (
    <div className="space-y-8 relative font-inter">
      ...
      <div className="flex items-center justify-between relative z-10">
        <div>
          <h1 className="font-dm-mono text-[30px] font-medium text-white tracking-tight flex items-center gap-4">
            Market Intelligence
            <span className="inline-flex items-center gap-2 px-2.5 py-1 text-[10px] border border-indigo-500/20 rounded-full text-indigo-400/80 bg-indigo-500/5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center border border-white/[0.07] rounded-lg overflow-hidden font-dm-mono text-[11px] tracking-widest">
            <button
              ...
              className={`px-4 py-2 transition-all ${isUS ? 'bg-indigo-500/15 text-white' : 'text-white/30 hover:text-white/60 hover:bg-white/5'}`}
            >
              $ US
            </button>
          ...
          <button
            onClick={doRefresh}
            className="px-4 py-2 rounded-lg border border-white/[0.07] bg-white/[0.02] backdrop-blur-xl text-white/40 hover:text-white/80 hover:bg-white/[0.06] transition-all font-dm-mono text-[11px] uppercase tracking-widest"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 relative z-10">
        {[
          ...
          <div key={card.label} className={card.glow ? CARD_GLOW : CARD}>
            <div className="font-inter text-[11px] text-white/30 uppercase tracking-[0.2em] mb-3 font-medium">{card.label}</div>
            <div className="font-dm-mono text-[28px] font-medium text-white mb-1.5 tabular-nums tracking-tight leading-none">{card.val}</div>
            <div className="font-dm-mono text-[10px] text-indigo-400/80 tracking-widest">{card.sub}</div>
          </div>
        ))}
      </div>
      ...
      <section className="relative z-10">
        <div className="flex items-center gap-3 mb-5">
          ...
          <span className="font-dm-mono text-[10px] text-white/25 uppercase tracking-[0.35em]">Market Indices · {isUS ? 'US' : 'INDIA'}</span>
        </div>
