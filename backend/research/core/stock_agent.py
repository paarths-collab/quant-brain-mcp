...
class StockSelectionAgent:
    ...
    def __call__(self, state: WealthState) -> WealthState:
        ...
        try:
            print("≡ƒöì Starting Stock Selection...")
            ...
            print(f"≡ƒÅå Winner: {best_ticker}")
            ...
            return {
                **state,
                ...
                "messages": [f"Γ£ô Selected Stock: {best_ticker} ({inv_mode})"]
            }
    ...
    def _deep_research_robust(self, ticker: str) -> Dict[str, Any]:
        ...
        try:
            print(f"   ≡ƒºá Asking Gemini for best search queries on {ticker}...")
        ...
        if self.tavily:
            try:
                ...
                print(f"   ΓÜá∩╕Å Tavily failed ({e}), trying fallback...")

        # 2. Try DuckDuckGo (Fallback A - Free, No API Key)
        if DDGS_AVAILABLE:
            try:
                print("   ≡ƒªå Trying DuckDuckGo...")
                with DDGS() as ddgs:
                ...
                print(f"   ΓÜá∩╕Å DuckDuckGo failed ({e}), trying fallback...")
        ...
        if news_key:
            try:
                ...
                print(f"   ΓÜá∩╕Å NewsDataIO failed ({e}), trying fallback...")
