...
class LLMManager:
    ...
    def __init__(self, temperature: float = 0.7):
        ...
        if not self.primary_llm:
            print("ΓÜá∩╕Å No Gemini keys found! Checking fallback providers...")
    ...
    def _rotate_key(self) -> bool:
        ...
        print(f"≡ƒöä Switching to Gemini Key #{self.current_key_index + 1}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "429" in error_str or "quota" in error_str or "resource exhausted" in error_str:
                    print(f"ΓÜá∩╕Å Quota/Rate Limit hit on key #{self.current_key_index + 1}. Attempting switch...")
                    if self._rotate_key():
                        time.sleep(1)  # Brief pause before retry
                        continue
                    else:
                        print("Γ¥î All Gemini keys exhausted. Use more keys in .env (GEMINI_API_KEY_3, etc.)")
                        raise e
                else:
                    # For non-quota errors, maybe simply retry or raise
                    print(f"Γ¥î LLM Error: {e}")
                    raise e
